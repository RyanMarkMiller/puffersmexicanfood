...
class LocationsPlugin extends GoldPlugin
{
	...
	function __construct()
	{
		...
		if( is_admin() && ( empty($_REQUEST['post_type']) || $_REQUEST['post_type'] !== 'location' ) && ($cur_post_type !== 'location') )
		{
			...
			if( $this->isValidKey() ){
				$Locations_MediaButton->add_button('Store Locator',  'store_locator', 'storelocatorwidget', 'location-alt');
			}
		}
	...
	{		
		...
		if (isset($_REQUEST['search_locations']) && isset($_REQUEST['your_location']) && strlen(trim($_REQUEST['your_location'])) > 0)
		{
			...
			$html .= $this->store_locator_results_html($your_location, $the_radius, $nearest_locations, $origin, $atts['show_search_results'], $atts['search_results_style']);
		}
		else 
		{
	...
	function store_locator_results_html($your_location, $radius, $nearest_locations, $origin, $show_search_results = true, $search_results_style = '')
	{
		$markers = array();
		$html = '';
				...
				foreach ( $nearest_locations as $loc ) {
					...
					switch( $search_results_style ) {
						case 'tile':
							...
							$item_html = '<li>';
							$item_html .= $this->build_location_html($loc_obj, $atts);
							$item_html .= '</li>';
							break;
						default:
	...
	function output_location_settings_page()
	{
		...
		$plugin_options[] = array(
			...
			'default' => 'locations',
			'desc' => 'The slug in the permalink when looking at Single Locations.  E.g, "http://www.yoursite.com/<strong>locations</strong>/roth-street-store".'
		);
...
<?php		
	...
	private function flush_location_rewrite_rules(){
...
<?php
	...
	function process_export()
	{
		...
		if (isset($_POST['_locs_pro_do_export']) && $_POST['_locs_pro_do_export'] == '_locs_pro_do_export') {
